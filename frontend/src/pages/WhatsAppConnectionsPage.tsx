...
function QRCountdown({ expiresAt }: { expiresAt: Date }) {
  ...
  return <span className="text-orange-600 text-sm">Expira em {timeLeft}s</span>;
}
...
export function WhatsAppConnectionsPage() {
  ...
  const [availableProviders, setAvailableProviders] = useState<Array<'WAHA' | 'EVOLUTION'>>([]);
  const [actionLoading, setActionLoading] = useState<string | null>(null);

  // Estados para modais de configuração
  const [settingsModalOpen, setSettingsModalOpen] = useState(false);
  const [connectionModeModalOpen, setConnectionModeModalOpen] = useState(false);
  const [selectedSession, setSelectedSession] = useState<WhatsAppSession | null>(null);

  // Estados para formulário de settings
        ...
        if (response.ok) {
          ...
          if (updatedSession) {
            ...
            if (updatedSession.status === 'WORKING' && updatedSession.me) {
              ...
              toast.success(
                `WhatsApp conectado com sucesso! Logado como: ${updatedSession.me.pushName}`
              );
            }
          }
        }
  ...
  const createSession = async () => {
    ...
    if (
      newSessionProvider === 'EVOLUTION' &&
      (!settings?.evolutionHost || !settings?.evolutionApiKey)
    ) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (
          ...
          (errorData.message && errorData.message.includes('Limite'))
        ) {
          toast.error(
            errorData.message ||
              'Limite de conexões atingido. Faça upgrade do seu plano para continuar.',
            {
              duration: 6000,
              icon: '⚠️',
            }
  ...
  const configureWebhook = async (session: WhatsAppSession) => {
    const sessionId = session.name; // Usar o name como ID
    setConfiguringWebhook(sessionId);
  ...
  // Handler para Logout (desconectar instância)
  const handleLogout = async (instanceName: string) => {
    setActionLoading(`logout-${instanceName}`);
    try {
      ...
      toast.success('Instância desconectada com sucesso!', { id: 'logout' });
      loadSessions();
    } catch (error) {
  ...
  // Handler para abrir modal de Modo de Conexão
  const openConnectionModeModal = (session: WhatsAppSession) => {
    setSelectedSession(session);
    setConnectionModeModalOpen(true);
  };

  // Handler para abrir modal de Settings
  ...
  const handleConfigureWebSocket = async (instanceName: string) => {
    setActionLoading(`websocket-${instanceName}`);
    try {
      ...
      toast.success('WebSocket configurado com sucesso!', { id: 'websocket' });
    } catch (error) {
      console.error('Erro ao configurar WebSocket:', error);
      toast.error(error instanceof Error ? error.message : 'Erro ao configurar WebSocket', {
        id: 'websocket',
      });
    } finally {
      setActionLoading(null);
  ...
  // Handler para submeter Settings
  const handleSubmitSettings = async () => {
    if (!selectedSession) return;

    setActionLoading(`settings-${selectedSession.name}`);
    try {
      ...
      toast.success('Definições configuradas com sucesso!', { id: 'settings' });
      setSettingsModalOpen(false);
    } catch (error) {
      console.error('Erro ao configurar definições:', error);
      toast.error(error instanceof Error ? error.message : 'Erro ao configurar definições', {
        id: 'settings',
      });
    } finally {
      setActionLoading(null);
  ...
  const openQRModal = async (sessionName: string) => {
    try {
      ...
      if (
        ...
        currentSession.qrExpiresAt > new Date()
      ) {
  ...
  return (
    <>
      ...
      <div className="p-6 space-y-6">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${session.provider === 'EVOLUTION' ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}
                        >
                      ...
                      {session.me && (
                        <p className="text-sm text-gray-600">
                          Conectado como: <span className="font-medium">{session.me.pushName}</span>{' '}
                          ({session.me.id})
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex gap-2 ml-4 flex-wrap">
                      ...
                      {/* Botões para instâncias Evolution conectadas */}
                      {session.status === 'WORKING' && session.provider === 'EVOLUTION' && (
                        <>
                          <button
                          ...
                          <button
                            ...
                            title="Configurar definições da instância"
                          >
                            ⚙️ Configurar
                          </button>
                        </>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            ...
            >
              <div className="text-center mb-8">
                ...
                <p className="text-gray-500 mt-2">Digite um nome para identificar esta sessão</p>
              </div>
              ...
              <div className="space-y-6">
                  ...
                  {availableProviders.length === 0 ? (
                    <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-xl">
                      ...
                      <p className="text-xs text-yellow-700 mt-1">
                        Configure WAHA ou Evolution nas configurações do sistema antes de criar uma
                        sessão.
                      </p>
                    </div>
                  ) : (
                    <>
                      <select
                        ...
                        onChange={(e) =>
                          setNewSessionProvider(e.target.value as 'WAHA' | 'EVOLUTION')
                        }
                ...
                <div className="flex gap-4 pt-6">
                  ...
                  <button
                    ...
                    disabled={
                      isCreating || !newSessionName.trim() || availableProviders.length === 0
                    }
                ...
                <div className="qr-modal-content bg-white rounded-lg shadow-xl">
                  <div className="flex justify-between items-center p-6 border-b">
                    <h3 className="text-lg font-medium text-gray-900">
                      QR Code - {currentQRSession.displayName || currentQRSession.name} (
                      {currentQRSession.provider})
                    </h3>
                      ...
                      <div className="text-center">
                        ...
                        <div className="bg-white p-6 rounded-lg border inline-block shadow-lg">
                          {(() => {
                            ...
                            if (
                              currentQRSession.provider === 'EVOLUTION' &&
                              currentQRSession.qr?.startsWith('data:image')
                            ) {
                              return (
                                <img
                            ...
                            if (
                              currentQRSession.provider === 'WAHA' &&
                              currentQRSession.qr?.startsWith('data:image')
                            ) {
                              return (
                                <img
        ...
        {/* Modal Configurar Settings */}
        {settingsModalOpen && selectedSession && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="p-6">
                <h3 className="text-xl font-bold mb-4">
                  ⚙️ Configurar Definições - {selectedSession.displayName || selectedSession.name}
                </h3>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      <p className="font-medium text-gray-900">Rejeitar Chamadas</p>
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          setInstanceSettings({ ...instanceSettings, rejectCall: e.target.checked })
                        }
                  ...
                  {instanceSettings.rejectCall && (
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setInstanceSettings({ ...instanceSettings, msgCall: e.target.value })
                        }
                  ...
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      <p className="font-medium text-gray-900">Ignorar Grupos</p>
                  ...
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      <p className="font-medium text-gray-900">Sempre Online</p>
                  ...
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      <p className="font-medium text-gray-900">Marcar como Lido</p>
                      <p className="text-sm text-gray-500">
                  ...
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      <p className="font-medium text-gray-900">Sincronizar Histórico Completo</p>
                      <p className="text-sm text-gray-500">Baixar todo histórico de conversas</p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        ...
                        onChange={(e) =>
                          setInstanceSettings({
                            ...instanceSettings,
                            syncFullHistory: e.target.checked,
                          })
                        }
                        className="sr-only peer"
                      />
                  ...
                  <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      <p className="font-medium text-gray-900">Marcar Status como Lido</p>
                      <p className="text-sm text-gray-500">
                        Marcar visualização de status automaticamente
                      </p>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        ...
                        onChange={(e) =>
                          setInstanceSettings({ ...instanceSettings, readStatus: e.target.checked })
                        }
                        className="sr-only peer"
                      />
                      <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-teal-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-teal-600"></div>
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button
                    ...
                    disabled={actionLoading === `settings-${selectedSession.name}`}
                  ...
                  >
                    {actionLoading === `settings-${selectedSession.name}`
                      ? '⏳ Salvando...'
        ...
        {/* Modal de Modo de Conexão */}
        {connectionModeModalOpen && selectedSession && (
          <ConnectionModeModal
            isOpen={connectionModeModalOpen}
            onClose={() => {
              setConnectionModeModalOpen(false);
              loadSessions(); // Recarregar sessões após alterações
            }}
